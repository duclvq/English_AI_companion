...
def _build_prompt(q: Question, chosen_index: int, level: str) -> str:
    ...
    if chosen_index < 0 or chosen_index >= len(q.choices):
        chosen_part = "User skipped this question (didn't know the answer)."
    else:
        chosen = q.choices[chosen_index]
        chosen_part = f"User chose: {chosen}. Explain why '{chosen}' is wrong."
    detail = {
    ...
    return (
        ...
        f"Correct answer: {correct}. {chosen_part}\n"
        f"Explain why '{correct}' is correct. "
...
async def stream_followup(q: Question, chosen_index: int, level: str, history: list[dict], user_question: str):
    ...
    correct = q.choices[q.correct_index]
    if chosen_index < 0 or chosen_index >= len(q.choices):
        chosen_part = "User skipped this question."
    else:
        chosen_part = f"User chose: {q.choices[chosen_index]}."
    system_msg = (
